...
**静屿是一座海上的小岛**：古琴五音疗愈、加密漂流日记、AI 树洞、情绪日历，还有一座随你心情慢慢生长的花田。无广告、无内购、无算法推荐——日记加密存储，即便数据库泄露也读不到明文。
...
|:---:|:---:|
| ![aihole](https://raw.githubusercontent.com/sunday-lil/jingyu/main/static/img/promo/jingyu_05_aihole.png) | ![garden](https://raw.githubusercontent.com/sunday-lil/jingyu/main/static/img/promo/jingyu_06_garden.png) |

## 跑起来

```bash
git clone https://github.com/sunday-lil/jingyu.git
cd jingyu
pip install -r requirements.txt
python start.py    # 浏览器打开 http://127.0.0.1:5000
```

首次启动会自动安装前端依赖并构建（需 Python 3.11+ 与 Node.js 18+，首次约几分钟），之后秒开；数据库与种子数据全自动，无需任何配置。
...
> 📜 **版本更新历史见 [CHANGELOG.md](CHANGELOG.md)**（v2.2.2 → v2.4.10 全部变更记录）。
...
> 日常使用只需上文「跑起来」的 4 行命令；以下命令用于本地开发、调试与服务器部署。

### 1.1 服务管理（[start.py](start.py)）

```bash
python start.py start      # 后台启动（默认 = 生产模式，FastAPI :5000 单进程）
python start.py --dev      # 应用/开发模式：Vite :5000（HMR）+ FastAPI :5001 一起起
python start.py --prod     # 显式生产模式（默认即生产，加不加效果一样）
python start.py stop       # 停止（同时停 FastAPI + Vite）
python start.py restart    # 重启
python start.py status     # 查看 PID + 端口（FastAPI / Vite 两个进程状态）
python start.py fg         # 前台运行（systemd / 调试用；可加 --dev 切应用模式）
python start.py build      # 构建前端到 static/dist/（自动 npm install + npm run build）
python start.py --init-db  # 启动前重置数据库
```

PID 写入 `run/healing.pid`（FastAPI）+ `run/vite.pid`（Vite），日志写入 `logs/healing.log` + `logs/vite.log`。服务器部署（宝塔 / systemd / Nginx）详见 [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md)。
...
前端代码独立到 [`frontend/`](frontend/) 目录，开发时用 Vite dev server 跑 SPA，热更新。v2.3.2 起 `python start.py` 默认为生产模式，应用/开发模式需显式 `--dev`（前后端一起起 + 自动 npm install）。

**推荐方式：`python start.py --dev` 一键起**（应用模式，自动 npm install 当 node_modules 不存在）

```bash
python start.py --dev    # 应用模式：自动起 Vite :5000（用户入口）+ FastAPI :5001（API）
