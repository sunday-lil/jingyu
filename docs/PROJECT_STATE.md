...
**最后更新**：2026-08-16（v2.4.7 音频方案回退 — 3 项改动：① **[REVERT] 移除 v2.4.6 合成音频**（删 generate_audio.py + 5 个 wav，`恢复mp3占位`）；② **[MIGRATION] audio_url 切回 .mp3**（`音频方案回退`，幂等反向迁移，老库重启自动切回）；③ **[PLAN] 待接入真实曲库**（`待接入真实曲库`，用户自备音频同名覆盖 static/audio/ 即可）。无新依赖，main.py 版本号 2.4.6→2.4.7。前一阶段 v2.4.6（2026-08-16 五音音频真实化尝试 — Karplus-Strong 合成古琴拨弦，**已被 v2.4.7 回退**）+ v2.4.5（2026-08-16 情绪日历 30 天趋势柱状图恢复 + 罗素情绪环显示修复 + 头像相册选择 + 通知空状态 emoji 统一 — 4 项修复）+ v2.4.4（2026-08-15 情绪日历透明修复 + 旧版日记迁移 + mood_checkins 主键重建 + 头像图片上传 + 落叶花坊文案打磨 — 10 项改动）+ v2.4.3（2026-08-14 花语文案焕新 + emoji 名称对齐 + 徽章奖励落叶 + 树洞三层回复 + 情绪日历空 bug 修复 — 14 项改动）+ v2.4.2（2026-08-13 整体架构优化与冗余清理，维护性清理版本 — 7 项改动）+ v2.4.1（2026-08-10 情绪日历改用罗素情绪环模型四象限图表 — 7 项改动）+ v2.4.0（2026-08-10 文案焕新 + 一天多条心情 + 头像/昵称编辑 + 花坊扩充 — 18 项改动）+ v2.3.3（2026-07-30 Safari 兼容性修复 — 3D 上下文恢复 + emoji 跨浏览器一致）+ v2.3.2（2026-07-28 start.py 默认生产模式 + 自动构建简化）+ v2.3（2026-07-25 六大四字名模块重构 + 双资源系统 + 花朵生命周期 + 通知 + 个人主页 + 古琴弹西洋曲谱 — 10 项大改））
...
| **v2.4.7 音频方案回退** | ✅ 完成 | 2026-08-16，3 项改动：**移除 v2.4.6 合成音频**（删 generate_audio.py + 5 个 wav）+ **恢复mp3占位**（5 个 5338 字节静音占位）+ **audio_url 幂等反向迁移 .wav→.mp3**；用户将自备真实音频（同名覆盖 static/audio/ 即可，零代码），main.py 版本号 2.4.7，详见 §2 |
