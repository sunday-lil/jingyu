...
| **v2.5.1 日常维护：GSAP 空目标警告清理** | ✅ 完成 | 2026-09-12，全站巡检（日志零 ERROR / 11 页扫描 0 失败请求）唯一遗留修复：**列表动画时机**（DiaryListView `.diary-item` / ShopView `.shop-group`+`.shop-card` 移至 fetch 成功后守卫播放 + AIChatView `.msg-row` 守卫）；同日完成**文档结构收纳**（HANDOFF/CHANGELOG/曲目清单入 docs/，1400+ 交叉链接修复，1463 链接复检全通过）；无新依赖/无迁移，main.py 版本号 2.5.1，详见 §2 |
