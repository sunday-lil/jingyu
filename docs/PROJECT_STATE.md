...
**最后更新**：2026-08-15（v2.4.3 补丁：首页滚动提示改可点击 `<button>` + 文案「向下沉入海面」→「向下，遇见岛上的去处」，纯前端交互修复，需重新 build。上一版 v2.4.3 花语文案焕新 + emoji 名称对齐 + 徽章奖励落叶 + 树洞三层回复 + 情绪日历空 bug 修复 — 14 项改动：① **删除「古琴初学者」废弃徽章**（`废弃徽章删除`）；② **「花田主人」→「花间客」**（`花间客改名`）；③ **「花坊」→「落叶花坊」**（`落叶花坊改名`）；④ **情绪日历空白 Bug 修复**（`情绪日历空白修复`，可选链 `cell.moodKeys?.length > 0`）；⑤ **落叶死锁解除**（`落叶死锁解除` / `BADGE_LEAF_REWARD=10`，每解锁一个徽章额外发放 10 落叶）；⑥ **花田 AI 显示基于实际种花**（`花田 AI 显示修复`，`v-if="flowers.length > 0"`）；⑦ **岛上物件 emoji 化** + **首页 emoji 🏝️→🌊** + **漂流瓶 emoji 🍶→🏺**；⑧ **树洞 AI 重写三层结构**（`树洞三层回复`）；⑨ **花种 emoji 与名称对齐 + 花语化**（`花语化` / `emoji 对齐`，12 种花种全改花语）；⑩ **装扮动物扩充**（`动物扩充`，小鸟🐦 / 小鸭🦆 / 小狗🐶）；⑪ **seed 改名迁移 + 去重**（`改名迁移` / `去重`）；⑫ **版本号 2.4.2 → 2.4.3**。无新依赖 / 不改后端模型结构（仅 seed 启动时改名 / 去重 / 删废弃行）。前一阶段 v2.4.2（2026-08-13 整体架构优化与冗余清理，维护性清理版本 — 7 项改动）+ v2.4.1（2026-08-10 情绪日历改用罗素情绪环模型四象限图表 — 7 项改动）+ v2.4.0（2026-08-10 文案焕新 + 一天多条心情 + 头像/昵称编辑 + 花坊扩充 — 18 项改动）+ v2.3.3（2026-07-30 Safari 兼容性修复 — 3D 上下文恢复 + emoji 跨浏览器一致）+ v2.3.2（2026-07-28 start.py 默认生产模式 + 自动构建简化）+ v2.3（2026-07-25 六大四字名模块重构 + 双资源系统 + 花朵生命周期 + 通知 + 个人主页 + 古琴弹西洋曲谱 — 10 项大改））
