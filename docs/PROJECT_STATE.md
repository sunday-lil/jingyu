...
> 🎵 **2026-08-16 v2.4.8 曲目独立音频架构（22 首曲目一曲一文件，替换五音共享 5 文件）**：用户指出核心架构问题——网页里有 22 首真实曲目（梅花三弄 / 流水 / 广陵散 / 卡农等），却按五音共享 5 个音频文件，「这么多曲目对应 5 个文件」根本没法按曲放置真实音频。① **[ARCH] 每曲独立文件**（`曲目独立音频`）：`audio_url` 从 `/static/audio/{五音}.mp3` 改为 `/static/audio/tracks/{曲名}.mp3`（22 个文件，中文曲名直接命名）——[seed.py](../app/seed.py) 新曲目直写 tracks 路径，`_ensure_placeholder_audio()` 改为按 SEED_MUSIC 逐曲生成静音占位（缺失才写）；② **[MIGRATION] 22 行 audio_url 重定向**（`audio_url迁移tracks`）：[database.py](../app/database.py) 幂等迁移 `UPDATE musics SET audio_url = '/static/audio/tracks/' || title || '.mp3'`（按曲名拼接，老库重启自动切换，已在 tracks/ 的行不受影响）；③ **[CHORE] 删除 5 个五音共享占位**（`五音共享废弃`）。**用户接入真实音频（零代码）**：下载对应曲目音频命名为曲名（如 `流水.mp3`）放入 `static/audio/tracks/` 同名覆盖即可，每首曲目独立对应、互不影响；前端播放器直接用 DB `audio_url`，零改动。详见 [HANDOFF §4 Phase 16](HANDOFF.md)。关键词 `v2.4.8` / `曲目独立音频` / `audio_url迁移tracks` / `五音共享废弃` 在 6 份文档中都要出现。

> 🏝️ **2026-09-13 v2.5.3 九项体验修正 + AI 模型切换（glm-5.3-flash）**：用户反馈集中修正 + AI 修复。① 首页删「🔒 日记端到端加密」信任行，「每一个情绪，都值得一座岛」→「每一种情绪，都值得温柔安放」（`首页文案微调`）。② 全站「漂流日记」→「日记海岸」，入口 emoji 📖（`日记海岸改名`，HomeView / AppLayout / islandGuide 三处同步）。③ 日记海岸内部 🏺→🍾（`瓶子emoji`，三视图）。④ 「古琴弹西洋」→「西洋曲谱」（emoji 🎼，卡上无曲名），西洋卡并入 [MusicListView.vue](../frontend/src/views/music/MusicListView.vue) 六宫格与五音同款外形（修复「宫」高出一截）；后端 `/api/music/yin/{yin}` 加 `category=="classic"` 过滤，西洋曲不混五音页（`西洋曲谱并入网格`）。⑤ 首页新增「我的」板块卡 👤（`首页我的卡`）。⑥ 「我的」页删「写一篇日记」按钮（`删写日记按钮`）。⑦ 情绪日历 emoji 🌙→🗓；新增打卡删除——「今日已记」每条带 ×，后端新增 `DELETE /api/mood/checkin/{id}`（仅删当天自己的，露水/徽章不回收）；30 天趋势模块移除（`打卡可删除`）。⑧ **AI 功能全灭修复**（`AI模型下架`）：NVIDIA NIM `meta/llama-3.1-8b-instruct` 已 2026-08-26 EOL 下架（410 Gone），AI 选音/树洞/鼓励语/治愈语全部降级「AI 暂时不在」——模型换 `z-ai/glm-5.3-flash`（.env + config.py 双改）+ [ai_service.py](../app/services/ai_service.py) `_call_nvidia` 加 3 次线性退避重试（超时/断连/429/5xx 重试，4xx 快速失败）+ reasoning 模型适配（思考内容计入 max_tokens，治愈语原 80 tokens 全被思考吃掉返回 content=None；全部调用提至 900-1200 + 空值容错）（`AI重试` / `reasoning模型适配`）。⑨ main.py 版本号 → 2.5.3。无新依赖 / 无 schema 迁移。浏览器端到端验证 9 项全部 PASS。关键词 `v2.5.3` / `日记海岸改名` / `西洋曲谱并入网格` / `打卡可删除` / `AI模型下架` / `glm-5.3-flash` / `AI重试` / `reasoning模型适配` 在 6 份文档中都要出现。

> 🎵 **2026-09-12 v2.5.2 真实曲库接入（16/22）+ 曲目时长自动校准**：用户从 Bilibili 下载 16 首古琴曲真实音频（10–28MB/首）同名覆盖进 `static/audio/tracks/`，音乐疗愈模块从占位音频变真实曲库（`真实曲库接入`；剩 6 首西洋改编仍占位）。① **《高山流水》→《流水》改名**：用户按俗名下载（B 站搜「流水」常出「高山流水」，同源传本异名），文件改名对齐 DB 曲名——曲名为准，不反向改 DB（保护 v2.4.8 title 幂等迁移链）。② **时长自动校准**（`时长自动校准` / `sync_durations_from_audio`）：[seed.py](../app/seed.py) 新增启动幂等校准——纯 stdlib MP3 时长解析（`_mp3_duration_seconds`：VBR 读 Xing 帧数 / CBR 按比特率估算），跳过 <100KB 占位文件（`占位文件跳过`），差值 ≥1s 才 UPDATE；16 首种子估值全不准（如大胡笳 706s 实际 vs 360s 估值），列表页时长从此与实际一致；**用户后续放剩余 6 首音频后重启即自动校准，零手工**（`数据自愈`）。SEED_MUSIC classic 16 首时长同步改实测值。破坏性验证：DB 改错 999s → 重启 → `[SYNC] 已按真实音频校准 1 首` → API 528.0 ✓；浏览器播放实测实时走秒 ✓。无前端改动 / 无新依赖 / 无 schema 迁移（main.py 版本号 → 2.5.2）。关键词 `v2.5.2` / `真实曲库接入` / `时长自动校准` / `sync_durations_from_audio` / `占位文件跳过` / `数据自愈` 在 6 份文档中都要出现。

> 🔧 **2026-09-12 v2.5.1 日常维护：GSAP 空目标警告清理（3 视图 4 处，纯前端）**：全站质量巡检（后端日志零 ERROR / 11 页浏览器扫描 0 个失败请求 / 写日记·打卡·AI 对话交互全通）发现唯一遗留——数据依赖型列表动画写在 `onMounted + nextTick` 时机，fetch 数据未到选择器为空，GSAP 报 "target not found" 警告且入场动画实际未播（`GSAP空目标`）。修复（`列表动画时机`，对齐 GardenView/NotificationsView 既有守卫模式）：① [DiaryListView.vue](../frontend/src/views/diary/DiaryListView.vue) `.diary-item` / ② [ShopView.vue](../frontend/src/views/garden/ShopView.vue) `.shop-group`+`.shop-card` 动画移至 fetch 成功后 `nextTick + querySelector` 守卫播放；③ [AIChatView.vue](../frontend/src/views/ai/AIChatView.vue) `.msg-row`（新对话挂载必无消息行）加守卫。新标签页复验三页 console 零消息；已 `npm run build`。**无后端改动 / 无数据库迁移 / 无新依赖**（main.py 仅版本号 → 2.5.1）。另：同日完成文档结构收纳（HANDOFF/CHANGELOG/曲目清单移入 docs/，1400+ 链接修复，1463 链接复检全通过）。关键词 `v2.5.1` / `列表动画时机` / `GSAP空目标` 在 6 份文档中都要出现。

> 🏝️ **2026-09-12 v2.5.0 首页动线与理念呈现重构第一期（纯前端）**：用户反馈首页动线不清晰、理念呈现不足，经 [HOME_REDESIGN_PROPOSAL.md](HOME_REDESIGN_PROPOSAL.md) v2 拍板：**主推 = 经营自己的小岛**（六模块并行同级），等级系统留第二期。① **Hero 理念文案卡**（`hero理念卡`）：[HomeView.vue](../frontend/src/views/HomeView.vue) hero 左品牌/右理念卡——「每一个情绪，都值得一座岛。」+ 心灵港湾三句 + 信任标记「🔒 日记端到端加密 · 无广告 · 无算法推荐」；移动端上下堆叠。② **模块卡奖励行**（`模块奖励行`）：六卡新增「✦ 怎么玩」一行（如「每听一曲 +1 露水 · 听满 10 首解锁「琴音知音」徽章」）。③ **岛上指南手风琴**（`岛上指南手风琴`）：首页新增「岛上指南」区块，六个折叠面板（`guideOpen` 展开收起），使用指南从「我的」前置到首页。④ **指南数据共享化**（`指南数据共享`）：ProfileView 本地 `GUIDE_SECTIONS` 提取至新文件 [frontend/src/data/islandGuide.js](../frontend/src/data/islandGuide.js)（`GUIDE_SECTIONS` + `PROFILE_GUIDE_SECTIONS`，新增 `reward` 字段，文案改一处两页同步）。⑤ **guest-cta 文案**（`开启我的岛`）：「开启静屿」→「开启我的岛」。⑥ GSAP 位移化 + 移动端响应式。**纯前端改动，无后端逻辑 / 无数据库迁移 / 无新依赖**（main.py 仅版本号 2.4.10 → 2.5.0），浏览器端到端验证全部 PASS。**第二期待办**：小岛等级系统（4 个待拍板决策点见提案 §六）。关键词 `v2.5.0` / `hero理念卡` / `模块奖励行` / `岛上指南手风琴` / `指南数据共享` / `开启我的岛` 在 6 份文档中都要出现。

> 🔧 **2026-08-23 v2.4.10 图标 viewBox 裁切修复（导航栏图标只显示左上角方块）**：用户报告「部分导航栏图标显示不全——只显示 emoji 左上角正方形范围」。根因（`图标左上角裁切`）：v2.4.9 生成的 [twemoji-icons.js](../frontend/src/assets/twemoji-icons.js) 图标条目只有 `body` 字段——`@iconify-json/twemoji` 的 icons.json 顶层才有 `width:36, height:36`（条目继承全局），`addIcon` 注册缺 width/height 时按 Iconify 默认 16×16 渲染 → `viewBox="0 0 16 16"` 与 twemoji 36×36 坐标的 path 不匹配 → 图形溢出视口，只露出左上角 16/36≈44% 区域（28 个图标全站受影响）。修复（`viewBox尺寸合并`）：[extract_twemoji.mjs](../scripts/extract_twemoji.mjs) 生成时显式合并顶层尺寸 `{width: src.width, height: src.height, ...data}`，重新生成 + 构建。浏览器实测：导航 10 个图标 viewBox 全部恢复 `0 0 36 36`，完整彩色图形显示。详见 [HANDOFF §4 Phase 18](HANDOFF.md)。关键词 `v2.4.10` / `viewBox尺寸合并` / `图标左上角裁切` 在 6 份文档中都要出现。

> 🛠️ **2026-08-17 v2.4.9 全站稳定性大修（6 个 bug 修复 + 项目结构清理）**：全站回归测试发现 6 个真实 bug 全部修复。① **[BUG FIX] GSAP 动画永久不可见全站根治**（`GSAP位移化`）：11 个视图（HomeView / MusicListView / MusicWesternView / AIChatView / ProfileView / GardenView / ShopView / NotificationsView / DiaryListView / DiaryWriteView / PickBottleView）的 `gsap.from()` 仍带 `opacity:0` 初始态，动画被中断（路由切换 / 切后台）时元素**永久卡在不可见状态**——「首页主标题不见」「/music 页整页空白」的根因；全部改为只保留位移 `y` 动画（v2.4.5 MoodCalendarView 同类修复的**全站推广**）。② **[BUG FIX] EmojiIcon 离线注册**（`emoji离线注册` / `twemoji-icons.js`）：[EmojiIcon.vue](../frontend/src/components/EmojiIcon.vue) 用 `<Icon>` 却从未注册图标数据，运行时逐个请求 `api.iconify.design`（断网 / 被墙图标全消失，与「离线 0 HTTP」注释矛盾）；新增 [scripts/extract_twemoji.mjs](../scripts/extract_twemoji.mjs) 从 `@iconify-json/twemoji`（10MB 全集）提取 28 个项目图标生成 [frontend/src/assets/twemoji-icons.js](../frontend/src/assets/twemoji-icons.js)（30KB），EmojiIcon 启动时 `addIcon` 注册——真正 **0 运行时 HTTP**。③ **[BUG FIX] 3 个错误图标名**（`图标名修正`）：EMOJI_MAP 中 `wave` / `gift` / `magnifying-glass-left` 在 twemoji 集合**不存在**（联网也 404）→ `water-wave` / `wrapped-gift` / `left-pointing-magnifying-glass`。④ **[BUG FIX] AudioVisualizer 崩溃**（`wavePhases作用域修复`）：`wavePhases` 声明在 `startRender()` 内部而 `renderWave()` 在外部引用 → 默认「流动波形」模式一渲染就 `ReferenceError`；提升为模块级常量。⑤ **[BUG FIX] 五音详情页 404**（`yin端点恢复`）：前端调 `GET /api/music/yin/{yin}`，该端点在历史重构中丢失（落入 SPA 兜底 404，「曲目加载失败」）→ [music.py](../app/routers/music.py) 补回，返回 `{musics:[...]}`。⑥ **[BUG FIX] axios 双重解包**（`双重解包修复`）：api 拦截器已解包 `response.data`，[MusicDetailView.vue](../frontend/src/views/music/MusicDetailView.vue) `res.data?.musics`（永远空列表）+ [MusicListView.vue](../frontend/src/views/music/MusicListView.vue) AI 推荐 `res.data || {}`（推荐永远失败）→ `res?.musics` / `res || {}`。⑦ **[CHORE] 结构清理**（`cookie_txt清理`）：删 git 追踪的 cookie.txt（curl 会话文件不该入库）+ `.gitignore` 增补。**无新依赖 / 无数据库迁移**，main.py 版本号 2.4.8 → 2.4.9。浏览器端到端回归：首页 / 音乐列表 / 五音详情（4 首可播放）/ 西方曲谱 / 日记 / 情绪日历打卡 / 花园 / 花坊 / 树洞 / 通知 / 个人主页全部 PASS。详见 [HANDOFF §4 Phase 17](HANDOFF.md)。关键词 `v2.4.9` / `GSAP位移化` / `emoji离线注册` / `twemoji-icons.js` / `图标名修正` / `wavePhases作用域修复` / `yin端点恢复` / `双重解包修复` / `cookie_txt清理` 在 6 份文档中都要出现。

**最后更新**：2026-09-13（v2.5.3 九项体验修正 + AI 模型切换 — 首页删加密信任行+主文案改「每一种情绪，都值得温柔安放」；全站「漂流日记」→「日记海岸」（📖 入口 / 🍾 内部瓶子）；「古琴弹西洋」→「西洋曲谱」（🎼）并入五音六宫格+后端 yin 端点 classic 过滤；首页新增「我的」卡；「我的」页删写日记按钮；情绪日历 🗓 + 打卡可删除（DELETE /api/mood/checkin/{id}）+ 30 天趋势移除；**AI 全灭修复**：NIM 模型 llama-3.1-8b 下架（410）→ 换 z-ai/glm-5.3-flash + _call_nvidia 3 次重试 + reasoning 模型 max_tokens 适配；浏览器验证 9 项全 PASS。无新依赖 / 无迁移，main.py 版本号 2.5.2→2.5.3。前一阶段 v2.5.2（2026-09-12 真实曲库接入 16/22 + 曲目时长自动校准 sync_durations_from_audio — 数据自愈：放音频重启自动对齐时长，零手工）+ v2.5.1（2026-09-12 GSAP 空目标警告清理 — 列表动画移至 fetch 成功后 + querySelector 守卫）+ v2.5.0（2026-09-12 首页动线与理念呈现重构第一期 — hero 理念卡 + 模块奖励行 + 岛上指南手风琴 + islandGuide.js 指南数据共享；第二期待办：小岛等级系统，4 个决策点见 docs/HOME_REDESIGN_PROPOSAL.md §六）+ v2.4.10（2026-08-23 图标 viewBox 裁切修复 — **viewBox尺寸合并**）+ v2.4.9（2026-08-17 全站稳定性大修 — 6 个 bug 修复 + 结构清理）+ v2.4.8（2026-08-16 曲目独立音频架构 — 22 首曲目一曲一文件）+ v2.4.7（2026-08-16 音频方案回退 — v2.4.6 合成方案试听否决，恢复 mp3 占位）+ v2.4.6（2026-08-16 五音音频真实化尝试 — Karplus-Strong 合成，**已被 v2.4.7 回退**）+ v2.4.5（2026-08-16 情绪日历 30 天趋势柱状图恢复 + 罗素情绪环显示修复 + 头像相册选择 + 通知空状态 emoji 统一 — 4 项修复）+ v2.4.4（2026-08-15 情绪日历透明修复 + 旧版日记迁移 + mood_checkins 主键重建 + 头像图片上传 + 落叶花坊文案打磨 — 10 项改动）+ v2.4.3（2026-08-14 花语文案焕新 + emoji 名称对齐 + 徽章奖励落叶 + 树洞三层回复 + 情绪日历空 bug 修复 — 14 项改动）+ v2.4.2（2026-08-13 整体架构优化与冗余清理，维护性清理版本 — 7 项改动）+ v2.4.1（2026-08-10 情绪日历改用罗素情绪环模型四象限图表 — 7 项改动）+ v2.4.0（2026-08-10 文案焕新 + 一天多条心情 + 头像/昵称编辑 + 花坊扩充 — 18 项改动）+ v2.3.3（2026-07-30 Safari 兼容性修复 — 3D 上下文恢复 + emoji 跨浏览器一致）+ v2.3.2（2026-07-28 start.py 默认生产模式 + 自动构建简化）+ v2.3（2026-07-25 六大四字名模块重构 + 双资源系统 + 花朵生命周期 + 通知 + 个人主页 + 古琴弹西洋曲谱 — 10 项大改））
...
| **v2.4.5 情绪日历 30 天趋势柱状图恢复 + 罗素情绪环显示修复 + 头像相册选择 + 通知空状态 emoji 统一** | ✅ 完成 | 2026-08-16，4 项修复：**30天趋势柱状图恢复**（恢复 v2.4.1 移除的柱状图板块，柱高 = 当日平均分 1-5，柱色心情渐变 + 柱顶 emoji + 未记录占位柱，与罗素情绪环并存）+ **罗素情绪环显示修复**（GSAP 动画只保留位移 `y`，不设 `opacity`/`scale` 初始态，根治动画中断永久不可见）+ **头像相册选择**（移除 `capture="environment"`，弹「拍照 / 从相册选择」选择框）+ **通知空状态emoji**（🌙→💛，与 v2.4.4 通知 emoji 统一对齐），纯前端 3 文件 + main.py 版本号 2.4.5，无迁移 / 无新依赖，详见 §2 |
| **v2.5.1 日常维护：GSAP 空目标警告清理** | ✅ 完成 | 2026-09-12，全站巡检（日志零 ERROR / 11 页扫描 0 失败请求）唯一遗留修复：**列表动画时机**（DiaryListView `.diary-item` / ShopView `.shop-group`+`.shop-card` 移至 fetch 成功后守卫播放 + AIChatView `.msg-row` 守卫）；同日完成**文档结构收纳**（HANDOFF/CHANGELOG/曲目清单入 docs/，1400+ 交叉链接修复，1463 链接复检全通过）+ **文档补漏**（README 徽章 / ARCHITECTURE / DEPLOYMENT 三份首提交漏更，用户质询后补齐）；依赖卫生：requirements.txt 零引用的 passlib[bcrypt] → bcrypt 直接依赖（无功能变化）/无迁移，main.py 版本号 2.5.1，详见 §2 |
| **v2.5.0 首页动线与理念呈现重构第一期** | ✅ 完成 | 2026-09-12，纯前端 5 项改动：**hero理念卡**（左品牌/右理念卡：「每一个情绪，都值得一座岛。」+ 心灵港湾三句 + 信任标记）+ **模块奖励行**（六卡「✦ 怎么玩」一行）+ **岛上指南手风琴**（首页折叠面板，使用指南前置）+ **指南数据共享**（islandGuide.js：GUIDE_SECTIONS + PROFILE_GUIDE_SECTIONS + reward 字段，ProfileView -79 行改共享源）+ **开启我的岛**（guest-cta 文案）；主推 = 经营自己的小岛（六模块并行同级），第二期小岛等级系统待拍板；无新依赖/无迁移，main.py 版本号 2.5.0，详见 §2 |
| **v2.4.10 图标 viewBox 裁切修复** | ✅ 完成 | 2026-08-23，1 项修复：**viewBox尺寸合并**（v2.4.9 的 twemoji-icons.js 图标条目缺 width/height，addIcon 按 Iconify 默认 16×16 渲染，viewBox 与 twemoji 36×36 坐标 path 不匹配 → 图标只显示左上角 44% 方块，28 个图标全站受影响；extract_twemoji.mjs 生成时合并顶层尺寸，重新生成 + 构建）；无新依赖/无迁移，main.py 版本号 2.4.10，详见 §2 |
| **v2.4.9 全站稳定性大修** | ✅ 完成 | 2026-08-17，6 个 bug 修复 + 结构清理：**GSAP位移化**（11 视图 `gsap.from()` 的 `opacity:0` 全部移除，根治动画中断永久不可见——首页标题不见/音乐页空白根因）+ **emoji离线注册**（twemoji-icons.js 28 图标 addIcon 注册，0 运行时 HTTP）+ **图标名修正**（wave/gift/magnifying-glass-left → water-wave/wrapped-gift/left-pointing-magnifying-glass）+ **wavePhases作用域修复**（AudioVisualizer ReferenceError）+ **yin端点恢复**（`GET /api/music/yin/{yin}` 补回）+ **双重解包修复**（res.data?.musics → res?.musics 等 2 处）+ **cookie_txt清理**；无新依赖/无迁移，main.py 版本号 2.4.9，详见 §2 |
| **v2.4.8 曲目独立音频架构** | ✅ 完成 | 2026-08-16，3 项改动：**曲目独立音频**（audio_url → `/static/audio/tracks/{曲名}.mp3`，22 个中文曲名文件，一曲一文件）+ **audio_url迁移tracks**（musics 表幂等迁移按 title 拼接，一条 SQL 重定向 22 行）+ **五音共享废弃**（删 5 个五音占位）；用户接入真实音频零代码（曲名同名覆盖 tracks/），前端零改动，main.py 版本号 2.4.8，详见 §2 |
| **v2.4.7 音频方案回退** | ✅ 完成 | 2026-08-16，3 项改动：**移除 v2.4.6 合成音频**（删 generate_audio.py + 5 个 wav）+ **恢复mp3占位**（5 个 5338 字节静音占位）+ **audio_url 幂等反向迁移 .wav→.mp3**；已被 v2.4.8 的 tracks 架构取代（五音占位再删除），详见 §2 |
| **v2.4.6 五音音频真实化** | ↩️ 已回退 | 2026-08-16，Karplus-Strong 合成古琴拨弦（`五音音频合成`）+ audio_url 迁移 .wav。**上线试听被否决：随机拨弦声 ≠ 成曲**，已被 v2.4.7 全量回退（详见 §2 v2.4.7 节） |
...
| **文档** | ✅ | README + 8 个 docs/（HANDOFF/CHANGELOG/曲目清单/HOME_REDESIGN_PROPOSAL/ARCHITECTURE/DEPLOYMENT/DEVELOPMENT/PROJECT_STATE，2026-09-12 收纳后根目录仅留 README），6 份文档同步（Iron Rule） |
...
- [x] **改动 6：新增 5 个五音封面 SVG**（`SVG封面`）：
  - `static/images/cover_gong.svg` / `cover_shang.svg` / `cover_jue.svg` / `cover_zhi.svg` / `cover_yu.svg`
...
- [x] **不动**：[static/css/](../static/css/) 全部保留（admin/_base.html 加载 style.css）/ [static/js/app.js](../static/js/app.js) 保留（仅改注释）/ [static/audio/](../static/audio/) 保留（seed.py 生成占位 mp3）/ [templates/admin/](../templates/admin/) 保留 / [config.py](../app/config.py) / [app/database.py](../app/database.py) / [requirements.txt](../requirements.txt) 不动
- [x] **数据库迁移**：无（不改后端模型 / 不改 API / 不改 service）
- [x] 文档同步（Iron Rule）：6 份文档同步更新 — README / HANDOFF / PROJECT_STATE / ARCHITECTURE / DEPLOYMENT / DEVELOPMENT
...
### 2026-08-16（v2.4.5）— 情绪日历 30 天趋势柱状图恢复 + 罗素情绪环显示修复 + 头像相册选择 + 通知空状态 emoji 统一

- [x] 起因：用户反馈 4 个问题——① 情绪日历打卡后下方不显示柱状图（v2.4.1 把柱状图换成了罗素情绪环，用户习惯看柱状趋势）；② 罗素情绪环模型不显示（GSAP 动画初始态残留 `opacity:0`/`scale:0`）；③ 头像只能拍照不能从相册选择（`capture` 属性强制调起相机）；④ 「我的」>「通知」空状态显示 🌙 而非 💛
- [x] **改动 1：[BUG FIX] 情绪日历打卡后柱状图不显示**（`30天趋势柱状图恢复`）：[MoodCalendarView.vue](../frontend/src/views/mood/MoodCalendarView.vue) 恢复「近 30 天心情趋势」柱状图板块（`.trend-section` 模板 + 样式 + `trend` computed），与罗素情绪环**并存**——柱高 = 当日心情平均分（1-5 评分，一天多条取平均），柱色取当日主心情颜色渐变，柱顶悬浮当日主心情 emoji（title 含 ×N），未记录日 3px 浅色占位柱，底部首尾日期轴
- [x] **改动 2：[BUG FIX] 罗素情绪环模型不显示**（`罗素情绪环显示修复`）：GSAP `from()` 动画残留 `opacity:0` / `scale:0` 初始态，动画被中断（切后台 / 路由切换）时元素永久卡在不可见状态（与 v2.4.4「透明 bug」同类根因，当时只修了心情按钮一处）；修复：全部入场动画只保留位移（`y`），不设 `opacity` / `scale` 初始态
- [x] **改动 3：[BUG FIX] 头像只能拍照不能从相册选择**（`头像相册选择`）：[ProfileView.vue](../frontend/src/views/profile/ProfileView.vue) 头像上传 `<input type="file">` 移除 `capture="environment"` 属性（保留 `accept="image/*"`），弹出系统「拍照 / 从相册选择」选择框，按钮文案改为「📷 拍照 / 从相册选择」
- [x] **改动 4：[BUG FIX] 通知空状态 emoji 错误**（`通知空状态emoji`）：[NotificationsView.vue](../frontend/src/views/notification/NotificationsView.vue) 空状态 emoji 🌙 → 💛，与 v2.4.4「通知 emoji 统一 💛」对齐（当时漏改空状态）
- [x] **Smoke test 结果**（2026-08-16 实测，dist 重构建后浏览器端到端验证）：
  - 打卡后 30 天趋势柱状图显示（柱高 / 柱色 / emoji 正确）✅
  - 罗素情绪环四象限正常渲染（不再被动画初始态卡住）✅
  - 头像上传弹「拍照 / 从相册选择」选择框（不再强制相机）✅
  - 通知空状态显示 💛 ✅
- [x] **改动文件**：仅 3 个前端文件（MoodCalendarView.vue / ProfileView.vue / NotificationsView.vue）+ [app/main.py](../app/main.py) 版本号 2.4.4 → 2.4.5（`版本号对齐`）；无数据库迁移 / 无新依赖 / 无后端逻辑改动
- [x] 文档同步（Iron Rule）：6 份文档同步更新 — README / HANDOFF / PROJECT_STATE / ARCHITECTURE / DEPLOYMENT / DEVELOPMENT

### 2026-08-16（v2.4.6）— 五音音频真实化（静音占位 → Karplus-Strong 合成古琴拨弦）

- [x] 起因：用户反馈项目里所有音频文件都是占位文件，实际播放不出声（原 `static/audio/*.mp3` 是 seed.py 生成的 5338 字节静音假 MP3）
- [x] **方案选型**：原计划从网上下载真实古琴录音，但沙箱把 curl / git / pip 网络全部静默拦截，GitHub 上也无古琴音频仓库（搜到的 Guqin-Dataset / guqinMM 均为谱本与训练代码）；改为**本地 Karplus-Strong 拨弦物理建模合成**——纯标准库、零版权风险、固定种子可复现
- [x] **改动 1：[FEATURE] 五音真实音频**（`五音音频合成`）：新增 `scripts/generate_audio.py`——宫 C 系统（中正平和）/ 商 D 系统（清肃）/ 角 E 系统（舒展）/ 徵 G 系统（明快）/ 羽 A 系统（柔润安宁）五声调式；低音区 + 慢速稀疏音符 + 长延音 + 18% 低八度散音 + 句间气口；每段 78 秒 / 22050Hz / 16bit / mono PCM WAV（3.4MB）
- [x] **改动 2：[MIGRATION] audio_url 切 .wav**（`audio_url切wav`）：[database.py](../app/database.py) musics 表块新增幂等迁移 `UPDATE musics SET audio_url = REPLACE(audio_url, '.mp3', '.wav') WHERE audio_url LIKE '%.mp3'`；[seed.py](../app/seed.py) 新曲目直写 `.wav`，占位兜底改写最小合法 RIFF 静音 WAV
- [x] **改动 3：[CHORE] 删除 5 个假 MP3 占位文件**
- [x] **Smoke test 结果**（2026-08-16 实测）：5 个 WAV `wave.getparams()` 校验全部通过（nchannels=1 / sampwidth=2 / framerate=22050 / nframes=1719900 = 78.0 秒）✅；前端 `<audio>` 使用 DB audio_url 无硬编码格式，浏览器播完 @ended 自动下一首 ✅
- [x] **改动文件**：scripts/generate_audio.py（新增）/ app/database.py / app/seed.py / static/audio/*.wav（5 个新增）/ 删 static/audio/*.mp3（5 个）/ app/main.py 版本号 2.4.5 → 2.4.6（`版本号对齐`）；**无新依赖**
- [x] 文档同步（Iron Rule）：6 份文档同步更新 — README / HANDOFF / PROJECT_STATE / ARCHITECTURE / DEPLOYMENT / DEVELOPMENT

### 2026-08-16（v2.4.7）— 音频方案回退（合成 wav 移除，恢复 mp3 占位，待接入真实曲库）

- [x] 起因：v2.4.6 的 Karplus-Strong 合成音频上线后用户试听反馈「不是音乐，就是播琴声」——**随机五声音符序列 ≠ 曲子**（无旋律动机、无乐句呼应、无起承转合），方案否决全量回退
- [x] **改动 1：[REVERT] 移除合成音频**：删除 `scripts/generate_audio.py` 与 5 个合成 wav，恢复 5 个 mp3 静音占位（`恢复mp3占位`，各 5338 字节 = ID3v2 头 + 50 静音帧 + ID3v1，与 v2.4.5 完全一致）
- [x] **改动 2：[MIGRATION] audio_url 切回 .mp3**（`音频方案回退`）：[database.py](../app/database.py) 幂等反向迁移 `UPDATE musics SET audio_url = REPLACE(audio_url, '.wav', '.mp3') WHERE audio_url LIKE '%.wav'`（跑过 v2.4.6 的库重启自动切回，日志 `[MIGRATE] musics.audio_url 已切回 .mp3`）；[seed.py](../app/seed.py) 新曲目恢复 `.mp3`、占位兜底恢复假 MP3 逻辑
- [x] **改动 3：[PLAN] 待接入真实曲库**（`待接入真实曲库`）：用户自备真实古琴音频，**同名覆盖 `static/audio/{gong|shang|jue|zhi|yu}.mp3` 即可，零代码改动**（22 首曲目按 yin_type 共享 5 个音频文件，沿用原架构）
- [x] **Smoke test 结果**（2026-08-16 实测）：5 个 mp3 占位恢复，字节数 5338 × 5 与 v2.4.5 完全一致 ✅；`git status` 确认 static/audio 下仅 5 个 mp3、无 wav 残留 ✅
- [x] **改动文件**：删 scripts/generate_audio.py / 删 static/audio/*.wav（5 个）/ 恢复 static/audio/*.mp3（5 个占位）/ app/database.py（反向迁移）/ app/seed.py（恢复 mp3）/ app/main.py 版本号 2.4.6 → 2.4.7（`版本号对齐`）；**无新依赖**
- [x] 文档同步（Iron Rule）：6 份文档同步更新 — README / HANDOFF / PROJECT_STATE / ARCHITECTURE / DEPLOYMENT / DEVELOPMENT

### 2026-09-12（v2.5.0）— 首页动线与理念呈现重构第一期（纯前端）

- [x] 起因：用户读完 README 反馈两点——① 网站需向初访者清晰介绍理念/关怀系统及模块间关系；② 首页使用动线不清晰、缺主推核心体验。经 [HOME_REDESIGN_PROPOSAL.md](HOME_REDESIGN_PROPOSAL.md) v2 拍板：**主推 = 经营自己的小岛**（六模块并行同级，不设单一主推按钮），等级系统留第二期
- [x] **改动 1：[FEATURE] Hero 理念文案卡**（`hero理念卡`）：[HomeView.vue](../frontend/src/views/HomeView.vue) hero 左品牌（🌊 + 潮声不止 + 静屿）/右理念卡——「每一个情绪，都值得一座岛。」+ 心灵港湾三句 + 信任标记「🔒 日记端到端加密 · 无广告 · 无算法推荐」；移动端上下堆叠
- [x] **改动 2：[FEATURE] 模块卡奖励行**（`模块奖励行`）：六卡 desc 下新增「✦ 怎么玩」一行（如「每听一曲 +1 露水 · 听满 10 首解锁「琴音知音」徽章」），与露水/徽章资源闭环呼应
- [x] **改动 3：[FEATURE] 岛上指南手风琴**（`岛上指南手风琴`）：首页新增「岛上指南」区块（模块区下方），六个折叠面板（`guideOpen` 展开收起 + guide-expand transition），使用指南从「我的」前置到首页，不跳页不打断
- [x] **改动 4：[ARCH] 指南数据共享化**（`指南数据共享`）：ProfileView 本地 `GUIDE_SECTIONS`（约 79 行）提取至新文件 [frontend/src/data/islandGuide.js](../frontend/src/data/islandGuide.js)——`GUIDE_SECTIONS`（六模块）+ `PROFILE_GUIDE_SECTIONS`（追加「我的」项），各条目新增 `reward` 字段；文案改一处两页同步
- [x] **改动 5：[COPY] guest-cta 文案**（`开启我的岛`）：未登录引导按钮「开启静屿」→「开启我的岛」（注册即得到自己的岛，呼应主推定位）
- [x] GSAP 入场动效遵循位移化铁律（只 `y` 位移）+ 移动端响应式（hero 纵排 / 指南卡紧凑）
- [x] **Smoke test 结果**（2026-09-12 浏览器端到端）：Hero 理念卡文案齐全 / 六模块卡 reward 行显示 / 岛上指南 6 张卡 DIV 手风琴交互（展开 4 条 details / 再点收起 / URL 不跳转）/ console 无 JS 错误，全部 PASS
- [x] **改动文件**：frontend/src/views/HomeView.vue + frontend/src/views/profile/ProfileView.vue + frontend/src/data/islandGuide.js（新增）+ docs/HOME_REDESIGN_PROPOSAL.md（新增设计稿）+ app/main.py 版本号 2.4.10 → 2.5.0（`版本号对齐`）+ README（badge / 理念句）；**无新依赖 / 无数据库迁移**，纯前端改动，已 `npm run build`
- [x] **第二期待办**：小岛等级系统——User 加 `island_level`/`island_exp` + 新建 level_service.py + 升级 toast + 花种四档门控（需先拍板 4 个决策点，见提案 §六）
- [x] 文档同步（Iron Rule）：6 份文档同步更新 — README / HANDOFF / PROJECT_STATE / ARCHITECTURE / DEPLOYMENT / DEVELOPMENT + CHANGELOG（第 7 份）

### 2026-08-23（v2.4.10）— 图标 viewBox 裁切修复（导航栏图标只显示左上角方块）

- [x] 起因：用户报告「网站上部分导航栏图标显示不全——只显示 emoji 左上角正方形的范围」（v2.4.9 引入的回归 bug）
- [x] **根因**（`图标左上角裁切`）：twemoji-icons.js 图标条目只有 `body` 字段——icons.json 顶层才有 `width:36, height:36`（条目继承全局）；`addIcon` 缺 width/height 时按 Iconify 默认 16×16 渲染 → `viewBox="0 0 16 16"` 与 twemoji 36×36 坐标的 path 不匹配 → 图形溢出视口，只露出左上角 16/36≈44% 区域（28 个图标全站受影响）
- [x] **修复**（`viewBox尺寸合并`）：extract_twemoji.mjs 生成时显式合并顶层尺寸 `out[name] = { width: src.width, height: src.height, ...data }`，重新生成 twemoji-icons.js（28/28 全带 36×36）+ npm run build
- [x] **Smoke test 结果**（2026-08-23 浏览器实测）：导航栏 10 个图标 SVG viewBox 全部恢复 `0 0 36 36`（width/height `1em`），铃铛/书本/水滴/樱花等完整彩色图形显示；首页模块卡片图标同步恢复正常；console 无报错
- [x] **改动文件**：scripts/extract_twemoji.mjs + frontend/src/assets/twemoji-icons.js（重新生成）+ app/main.py 版本号 2.4.9 → 2.4.10（`版本号对齐`）；无新依赖 / 无迁移
- [x] 文档同步（Iron Rule）：6 份文档同步更新 — README / HANDOFF / PROJECT_STATE / ARCHITECTURE / DEPLOYMENT / DEVELOPMENT

### 2026-08-17（v2.4.9）— 全站稳定性大修（6 个 bug 修复 + 项目结构清理）

- [x] 起因：用户要求全面测试找 bug。浏览器端到端回归（注册 → 登录 → 逐模块走查）揪出 6 个真实 bug
- [x] **改动 1：[BUG FIX] GSAP 动画永久不可见全站根治**（`GSAP位移化`）：11 个视图（HomeView / MusicListView / MusicWesternView / AIChatView / ProfileView / GardenView / ShopView / NotificationsView / DiaryListView / DiaryWriteView / PickBottleView）`gsap.from()` 的 `opacity:0` 全部移除，只保留位移 `y`——「首页主标题不见」「/music 整页空白」的根因（v2.4.5 只修了 MoodCalendarView 一处，本次全站推广）
- [x] **改动 2：[BUG FIX] EmojiIcon 离线注册**（`emoji离线注册`）：新增 scripts/extract_twemoji.mjs 从 @iconify-json/twemoji 提取 28 个图标生成 frontend/src/assets/twemoji-icons.js（30KB），EmojiIcon.vue `addIcon` 启动注册——修复运行时逐个请求 api.iconify.design（断网图标全消失）
- [x] **改动 3：[BUG FIX] 3 个错误图标名**（`图标名修正`）：wave / gift / magnifying-glass-left 在 twemoji 集合不存在 → water-wave / wrapped-gift / left-pointing-magnifying-glass
- [x] **改动 4：[BUG FIX] AudioVisualizer 崩溃**（`wavePhases作用域修复`）：wavePhases 从 startRender() 内部提升为模块级常量（renderWave 外部作用域引用报 ReferenceError）
- [x] **改动 5：[BUG FIX] 五音详情页 404**（`yin端点恢复`）：app/routers/music.py 补回 `GET /api/music/yin/{yin}`（返回 `{musics:[...]}`，注册在 /{music_id} 之前）
- [x] **改动 6：[BUG FIX] axios 双重解包**（`双重解包修复`）：MusicDetailView `res.data?.musics` → `res?.musics`；MusicListView `res.data || {}` → `res || {}`（拦截器已解包 response.data）
- [x] **改动 7：[CHORE] 结构清理**（`cookie_txt清理`）：删 git 追踪的 cookie.txt + .gitignore 增补
- [x] **Smoke test 结果**（2026-08-17 浏览器实测）：首页 / 音乐列表 / 五音详情（宫音 4 首加载+选中播放）/ 西方曲谱 / 日记 / 情绪日历打卡 / 花园 / 花坊 / 树洞 / 通知 / 个人主页全部 PASS，console 无 iconify 外网请求、无 ReferenceError
- [x] **改动文件**：11 个 .vue 视图 + EmojiIcon.vue + AudioVisualizer.vue + twemoji-icons.js（新增）+ extract_twemoji.mjs（新增）+ app/routers/music.py + .gitignore + 删 cookie.txt / app/main.py 版本号 2.4.8 → 2.4.9（`版本号对齐`）；**无新依赖**
- [x] 文档同步（Iron Rule）：6 份文档同步更新 — README / HANDOFF / PROJECT_STATE / ARCHITECTURE / DEPLOYMENT / DEVELOPMENT

### 2026-08-16（v2.4.8）— 曲目独立音频架构（22 首曲目一曲一文件）

- [x] 起因：用户指出核心架构问题——网页有 22 首真实曲目（梅花三弄 / 流水 / 广陵散 / 卡农等），却按五音共享 5 个音频文件，「这么多曲目对应 5 个文件」没法按曲放置真实音频——`audio_url` 粒度（五音）与展示粒度（曲目）不一致的架构债
- [x] **改动 1：[ARCH] 每曲独立文件**（`曲目独立音频`）：`audio_url` 从 `/static/audio/{五音}.mp3` 改为 `/static/audio/tracks/{曲名}.mp3`（22 个文件，**中文曲名直接命名**，用户下载音频不用改名）；[seed.py](../app/seed.py) 新曲目直写 tracks 路径，`_ensure_placeholder_audio()` 按 SEED_MUSIC 逐曲生成静音占位（缺失才写）
- [x] **改动 2：[MIGRATION] 22 行 audio_url 重定向**（`audio_url迁移tracks`）：[database.py](../app/database.py) 幂等迁移 `UPDATE musics SET audio_url = '/static/audio/tracks/' || title || '.mp3' WHERE audio_url LIKE '/static/audio/%.mp3' AND audio_url NOT LIKE '/static/audio/tracks/%'`（按 title 拼接，老库重启自动切换，日志 `[MIGRATE] musics.audio_url 已切换为每曲独立文件`）
- [x] **改动 3：[CHORE] 删除 5 个五音共享占位**（`五音共享废弃`，audio 根目录只剩 tracks/ 子目录）
- [x] **前端零改动**：两个播放页都用 `<audio :src="currentMusic.audio_url">` + `@ended` 下一首，DB 驱动
- [x] **Smoke test 结果**（2026-08-16 实测）：22 个占位文件生成，Glob 校验中文文件名全部正确（各 5338 字节）✅；audio 根目录仅 tracks/ 子目录、无五音残留 ✅
- [x] **改动文件**：app/seed.py / app/database.py / static/audio/tracks/*.mp3（22 个新增）/ 删 static/audio/*.mp3（5 个）/ app/main.py 版本号 2.4.7 → 2.4.8（`版本号对齐`）；**无新依赖**
- [x] 文档同步（Iron Rule）：6 份文档同步更新 — README / HANDOFF / PROJECT_STATE / ARCHITECTURE / DEPLOYMENT / DEVELOPMENT
...
- [x] **改动 2：Google Fonts 换国内镜像**
  - `templates/base.html`：3 行字体引用（preconnect + link）从 `fonts.googleapis.com` / `fonts.gstatic.com` 改为 `fonts.loli.net` / `gstatic.loli.net`
...
- [x] **前端集成 4 处**：
  - **AI 树洞对话**：新增 `templates/ai_chat.html` + `static/js/pages/ai_chat.js`，独立页面 `/ai-chat`（需登录），多轮对话历史只存浏览器内存，刷新清空，**不落库**
  - **漂流瓶 AI 鼓励语**：`templates/pick_bottle.html` 加 `#ai-encouragement` 容器 + `static/js/pages/pick.js` 加 `loadAIEncouragement`，拾瓶成功后调 `/api/ai/encouragement`；AI 文案给读者看，**不写库**，不污染作者收件箱
  - **情绪日历 AI 治愈语**：`templates/mood_calendar.html` 加 `#ai-healing-msg` 容器 + `static/js/pages/mood_calendar.js` 加 `loadAIHealing`，打卡成功后调 `/api/ai/healing`，显示在今日心情卡片下方，**不落库**
  - **音乐 AI 心情推荐**：`templates/index.html` 加「AI 帮我选音」卡片（仅登录可见）+ 新建 `static/js/pages/home.js`，调 `/api/ai/recommend-music`，推荐宫商角徵羽之一 + 理由 + 跳转 `/music/{yin}`
...
- [x] **需求 1：密码可见性切换**
  ...
  - `templates/login.html` + `templates/register.html`：密码 input 包裹 `.password-input-wrap`，加 `<button class="password-toggle" data-target="password">👁</button>`
  - `static/js/pages/diary.js`：`askPassword` 动态 modal 的 input 同样包裹 + toggle 按钮
...
- [x] **需求 2：iOS 导航栏优化 + 退出按钮可见性**
  ...
  - `templates/_nav.html`：L13 `/mood` → `/mood-calendar`，"手帐" → "日历"（修会话 6 遗漏未改的桌面 nav 链接）
  - `templates/base.html` + `templates/index.html`：meta description / hero subtitle 文案 "心情手帐" → "情绪日历"
- [x] **需求 3：情绪日历删文本输入区**
  - `templates/mood_calendar.html`：删除 textarea #mood-note + form-hint 整段；文案改为 "选一个表情，记录今天的心情"
  - `static/js/pages/mood_calendar.js`：删除 noteEl 取值，提交 `note: null`；文件头注释更新 "2026-07-16 移除文本输入，甲方要求文字内容统一进日记模块"
  - **数据迁移零改动**：DB 查询确认 `MoodCheckin.note` 历史数据 `with_note: 0`（本就 nullable=True，无历史数据需要迁移）
- [x] **需求 4：日历日期数字替换为情绪 emoji**
  - `static/js/pages/mood_calendar.js` `renderCalendar`：`isChecked` 时 content 只生成 `<span class="mood-emoji">${emoji}</span>`，否则显示数字；title 显示日期
...
- [x] **需求 5：日记编辑页删心情选择模块**
  - `templates/diary_write.html`：删除整个心情选择模块（page-header + mood-grid）；placeholder 加 "也可以贴任何 emoji 🌸" 暗示
  - `static/js/pages/diary.js`：删除 `selectedMood` + moodItems click listener，提交 `mood_type: null`；**`Diary.mood_type` 字段保留**（向后兼容历史数据，新日记为 null）
...
- [x] 改 `templates/mood_calendar.html`：顶部新增「今日心情」卡片（表情网格 + 可选备注 textarea + 收好按钮），文案「只选一个表情也行，文字想什么时候写就什么时候写」；连胜卡文案改为「连续记满 7 天有奖励，不勉强每天都来 ☀️」；删除趋势卡里的「记今天」按钮
- [x] 改 `static/js/pages/mood_calendar.js`：合并原 `mood.js` 的打卡逻辑（moodItems 选择 + saveBtn 保存 + confetti/floatEnergy 反馈），保存成功后调用 `loadCalendar()` + `loadTrend()` 刷新今日格子、趋势、连胜
...
- [x] 改 `templates/index.html`：删除首页 `{% if current_user %}<!-- 今日手帐条 -->{% endif %}` today-strip section；删除「今日手帐」module-card；更新「情绪日历」module-card 文案为「记今日心情，把日子染成颜色。不勉强每天，想记就记。」，图标渐变改为黄绿（#FFD56B → #A8D5BA）；漂流瓶相关卡保留不动，与情绪日历分开
- [x] 改 `templates/base.html` tabbar：`/mood` → `/mood-calendar`，图标 🌱 → 📅，文案「手帐」→「日历」，加 `is-active` 判断
- [x] 删除 `templates/mood_checkin.html` + `static/js/pages/mood.js`（不再使用，逻辑已合并进 mood_calendar.* ）
...
- [x] 改 `templates/base.html`：加 `.petal-layer` 花瓣层 + `<main class="page-transition">`
- [x] 改 `templates/index.html`：标题 `title-shimmer`、五音 / 模块区容器 `reveal`（容器级揭示，避免与卡片 hover transform 冲突）
- [x] 改 `templates/shop.html`：能量数字 `data-countup` 计数
- [x] 改 `templates/music_list.html` + `static/js/pages/music.js`：播放器内嵌 `.eq-bars` 频谱（播放/暂停切换 `.is-active`）、列表容器 `reveal`
- [x] 改 `static/js/pages/shop.js` + `static/js/pages/mood.js`：成功时 `QI.confetti()` 撒花瓣
